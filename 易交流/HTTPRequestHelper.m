...
+ (NSString *)HTTPnetworking_md5:(NSString *)string;
...
static inline NSString *cachePath() {
    return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/HTTPNetworkingCaches"];
}
...
+ (void)clearCaches {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:directoryPath isDirectory:nil]) {
        ...
        if (error) {
            NSLog(@"HTTPNetworking clear caches error: %@", error);
        } else {
            NSLog(@"HTTPNetworking clear caches ok");
        }
    }
}
...
+ (HTTPSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
    ...
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil) {
            KaelAppLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
        }
    } else {
    ...
    if (httpMethod == 1) {
        if (sg_cacheGet) {
            if (sg_shoulObtainLocalWhenUnconnected) {
                if (sg_networkStatus == kHTTPNetworkStatusNotReachable ||  sg_networkStatus == kHTTPNetworkStatusUnknown ) {
                    id response = [HTTPRequestHelper cahceResponseWithURL:absolute
                                                           parameters:params];
                    if (response) {
                        if (success) {
        ...
        session = [manager GET:url parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
...
+ (HTTPSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        KaelAppLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        KaelAppLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
...
+ (HTTPSessionTask *)uploadWithImage:(UIImage *)image
    ...
    AFHTTPSessionManager *manager = [self manager];
    HTTPSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (HTTPSessionTask *)downloadWithUrl:(NSString *)url
    ...
    session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (error == nil) {
            ...
            if ([self isDebug]) {
                KaelAppLog(@"Download success for url %@",
            ...
            if ([self isDebug]) {
                KaelAppLog(@"Download fail for url %@, reason : %@",
                          [self absoluteUrlWithPath:url],
                          [error description]);
            }
        }
...
+ (AFHTTPSessionManager *)manager {
    ...
    [manager.requestSerializer setValue:@"1"forHTTPHeaderField:@"api-version"];
    //        [_manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"content-type"];
    //        [_manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Accept"];
    [manager.requestSerializer setValue:@"" forHTTPHeaderField:@"t_type"];
    [manager.requestSerializer setValue:@"" forHTTPHeaderField:@"regionCode"];
    
    [manager POST:@"URL" parameters:[NSDictionary dictionary] progress:^(NSProgress * _Nonnull uploadProgress) {
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        NSString *key = [NSString HTTPNetworking_md5:absoluteURL];
        NSString *path = [directoryPath stringByAppendingPathComponent:key];
        
        NSData *data = [[NSFileManager defaultManager] contentsAtPath:path];
        if (data) {
            cacheData = data;
            KaelAppLog(@"Read data from cache for url: %@\n", url);
        }
    }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
    if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
            ...
            [[NSFileManager defaultManager] createDirectoryAtPath:directoryPath
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error];
            if (error) {
                KaelAppLog(@"create cache dir error: %@\n", error);
                return;
            }
        }
        ...
        if (data && error == nil) {
            ...
            if (isOk) {
                KaelAppLog(@"cache file ok for request: %@\n", absoluteURL);
            } else {
                KaelAppLog(@"cache file error for request: %@\n", absoluteURL);
            }
        }
    }
}
